...
export class UndoTreeManager implements vscode.Disposable {
    ...
    private diffBuffer = new Map<string, Diff[]>(); // URI → 保存前の差分バッファ
    private nextId = 1;
    ...
    onDidChangeTextDocument(e: vscode.TextDocumentChangeEvent) {
        ...
        for (const change of e.contentChanges) {
            buffer.push({
                ...
                removeLength: change.rangeLength,
    ...
    private shouldStoreFull(diffs: Diff[], contentLength: number): boolean {
        ...
        const changedChars = diffs.reduce((sum, d) => sum + d.removeLength + d.inserted.length, 0);
    ...
    private applyDiffs(content: string, diffs: Diff[]): string {
        // offsetの降順に適用（後ろから変更することでoffsetのズレを防ぐ）
        const sorted = [...diffs].sort((a, b) => b.offset - a.offset);
        let result = content;
